...
def run(content_dataset, style_dataset, dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    ...
    G_loss    = EasyDict(func_name='training.loss.G_logistic_ns_pathreg_st')   # Options for generator loss.
    D_loss    = EasyDict(func_name='training.loss.D_logistic_r1_st')           # Options for discriminator loss.
