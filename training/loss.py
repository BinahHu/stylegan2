...
def G_logistic_ns_pathreg_st(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    _ = opt
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    # TODO: use true loss
    latent_content = tf.random_normal([minibatch_size] + G.input_shapes[2][1:])
    fake_images_out, fake_dlatents_out = G.get_output_for(latents, labels, latent_content, is_training=True, return_dlatents=True)
    fake_scores_out = D.get_output_for(fake_images_out, labels, is_training=True)
    loss = tf.nn.softplus(-fake_scores_out) # -log(sigmoid(fake_scores_out))
...
def D_logistic_r1_st(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    _ = opt, training_set
    # TODO: use true loss
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    latent_content = tf.random_normal([minibatch_size] + G.input_shapes[2][1:])
